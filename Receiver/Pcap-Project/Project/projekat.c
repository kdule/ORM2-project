...
// Datoteka: Projektni zadatak (vezba6.c)
...
int main()
{
	...
	/************************************************************************************************************************************/
	// Open the capture device		ETHERNET


	if ((device_handle_ethernet = pcap_open_live(device_ethernet->name,		// name of the device
...
#ifdef _WIN32
	if (device_ethernet->addresses != NULL)
		/* Retrieve the mask of the first address of the interface */
		netmask = ((struct sockaddr_in *)(device_ethernet->addresses->netmask))->sin_addr.S_un.S_addr;
	else
		/* If the interface is without addresses we suppose to be in a C class network */
		netmask = 0xffffff;
#else
	if (!device_ethernet->addresses->netmask)
		netmask = 0;
	else
		netmask = ((struct sockaddr_in *)(device_ethernet->addresses->netmask))->sin_addr.s_addr;
#endif

	/************************************************************************************************************************************/
	// Compilint and setting all filters

	// Compile the filter
	if (pcap_compile(device_handle_ethernet, &fcode, filter_exp, 1, netmask) < 0)
	{
		printf("\n Unable to compile the packet filter. Check the syntax.\n");
		return -1;
	}
	else {
		printf("Compiled the ethenet device handle filter!\n");
	}

	// Set the filter
	if (pcap_setfilter(device_handle_ethernet, &fcode) < 0)
	{
		printf("\n Error setting the filter.\n");
		return -1;
	}
	else {
		printf("Set the ethenet device handle filter!\n");
	}
	...
	if (pcap_compile(device_handle_wifi, &fcode, filter_exp, 1, 0xffffff) < 0)
	{
		...
		printf("Compiled the wifi device handle filter!\n");
	}
	...
	if (pcap_setfilter(device_handle_wifi, &fcode) < 0)
	{
		...
		printf("Set the wifi device handle filter!\n");
	}


	printf("\nListening on %s...\n", device_ethernet->description);
	...
	pcap_loop(device_handle_ethernet, 0, packet_handler2, NULL);
	...
	if (pcap_sendpacket(device_handle_ethernet, ACK, len) == -1)
	{
	...
	pcap_loop(device_handle_ethernet, 0, packet_handler_eth, NULL);
	
	pcap_loop(device_handle_wifi, 0, packet_handler_wifi, NULL);																										//zakomentarisan pcap_loop za wifi
	
	//listenOverWiFi();																																					//i poziv listenera
	...
	pcap_close(device_handle_ethernet);
...
void packet_handler_eth(unsigned char *param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	printf("Entered the packet_handler_eth\n");
	...
	if (ih->src_addr[0] == 192 && ih->src_addr[1] == 168 && ih->src_addr[2] == 106 && ih->src_addr[3] == 1 && src_port == 8080 && strcmp(custom_header, "ORM2Proj") == 0)
	{
		...
		//printf("AAAAAAAAAAAAAAAAAA ETHERNET AAAAAAAAAAAAAAAAAAAAA\n");
	}

		if (i == (NMBR_OF_PACKETS / 2))
			pcap_breakloop(device_handle_ethernet);
...
void packet_handler_wifi(unsigned char *param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	printf("Entered the packet_handler_wifi\n");
	unsigned short src_port;
	...
	// Check the type of ethernet data
	if (ntohs(eh->type) != 0x0800){ // Ipv4 = 0x0800
		printf("Problem with ntohs(eh->type) != 0x0800 \n");
		return;
	}
	...
	// Check the type of ip data
	if (ih->next_protocol != 0x11) { // UDP = 0x11
		printf("Problem with ih->next_protocol != 0x11 \n");
		return;
	}
	...
	if (ih->src_addr[0] == 192 && ih->src_addr[1] == 168 && ih->src_addr[2] == 123 && ih->src_addr[3] == 18 && src_port == 8080 && strcmp(custom_header, "ORM2Proj") == 0)
	{
		...
		//printf("AAAAAAAAAAAAAAAAAA WIFI AAAAAAAAAAAAAAAAAAAAA\n");
	}
...
void packet_handler2(unsigned char *param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)																		// dodatni packet_handler za provere
{
	...
	if (j == 0 && strcmp(custom_header, "ORM2Proj") == 0)
	{
		fileName = (packet_data + 56);
		j++;
	}
	else if (j == 1 && strcmp(custom_header, "ORM2Proj") == 0)
	{
